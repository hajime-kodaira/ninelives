...
| `-extra` | false | ツールが認識していない枠も行に出す（Claude。Codex では何もしない） |
...
このエンドポイントは `limits` 配列のほかに、`omelette_promotional` や `amber_ladder` といったコードネーム付きのフィールドを大量に返します。ほとんどは `null` ですが、期間限定の増枠のようなものが将来ここに現れる可能性があります。

- **`limits` に新しい `kind` が増えた場合は、何もしなくても行が増えます。** 知らない `kind` はその名前のままラベルになります。
- **トップレベルに知らない枠が現れた場合は、初回だけ標準エラーに通知します。** 常駐させていれば `~/Library/Logs/ninelives.log` に残ります。表示に加えるには `-extra` を付けてください。

現在この口座では `nimbus_quill` が（`null` ではなく利用率 0 で）返ってきています。

```
$ ninelives -stdout -extra -credits
  5h            46% left · 1h36m
  7d            87% left · 2d20h
  7d Fable      80% left · 2d20h
  nimbus_quill  100% left
  Credits       $7.35 used        ← 金額なのでバーは出ません
```

メニューバーの数字は常にプラン本体の枠だけを見ます。認識していない枠が 0% だからといって、バーが満タンに見えることはありません。
